...
import com.grupogbs.bot.entities.dtos.whatsapp.EnvioWhatsappDTO;
import com.grupogbs.bot.entities.dtos.whatsapp.RespostaWhatsappDTO;
...
public class WhatsappService {
	...
	public HttpStatusCode enviarMensagemAtualizacaoOS(OrdemServicoDTO os) throws JsonProcessingException {
		...
		EnvioWhatsappDTO body = new EnvioWhatsappDTO(os, sender, templateId);
		...
		// Base URL.
		String baseUrl = "https://api.zenvia.com/v2/channels/whatsapp/messages";
		
		try {
			
			// Fazendo o request e aguardando a resposta.
			ResponseEntity<RespostaWhatsappDTO> response = restTemplate
					.exchange(
							(baseUrl),
							HttpMethod.POST,
							entity, 
							RespostaWhatsappDTO.class
							);
			
			
			// Transformando os dados e devolvendo os resultados.
			RespostaWhatsappDTO r = response.getBody();
			System.out.println("\n\n Resposta da API : \n\n");
			System.out.println(r);
			
			if(response.getStatusCode().equals(HttpStatusCode.valueOf(200))) {
				mensagemService.registrarEnvioMensagem(os, Servico.WHATSAPP);
			}
